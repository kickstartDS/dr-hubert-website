// EventHeaderComponent.tsx renders `TagLabel` from `@kickstartds/base`, which
// relies on the vanilla-JS `TagLabel` behavior (`base.tag-label`) for its
// remove-button click handling. That registration is only reachable from the
